            ...
            for i in directory[:]:
                if i[fs.A_NAME] == basename:
                    if i[fs.A_TYPE] == fs.T_DIR and not recursive:
                        ...
                        log.msg(f"[DEBUG][commands.py][fs.py] Removing file: {i[fs.A_NAME]}", system="cowrie")
                        log.msg(f"[DEBUG][commands.py][fs.py] pname PATH: {pname}", system="cowrie")
                        
                        if self.fs.extract_username_from_authorized_keys(pname):
                            self.fs.update_size(pname, i[fs.A_SIZE], True)
...
class Command_cp(HoneyPotCommand):
    ...
    def call(self) -> None:
        ...
        for src in sources:
            ...
            if isdir:
                outfile_path = os.path.join(resolv(dest), outfile)
            else:
                outfile_path = resolv(dest)
            
            s[fs.A_NAME] = outfile
            log.msg(f"[DEBUG][commands.py][fs.py] Copying file: {outfile}", system="cowrie")
            log.msg(f"[DEBUG][commands.py][fs.py] FROM file: {resolv(src)}", system="cowrie")
            ...
            if s[fs.A_REALFILE]:
            # Create a new real file with 'cp' appended
                original_realfile = s[fs.A_REALFILE]
                new_realfile = original_realfile + "_cp"

                try:
                    shutil.copyfile(original_realfile, new_realfile)
                    log.msg(f"[DEBUG][cp.py] Copied realfile from {original_realfile} to {new_realfile}", system="cowrie")
                except Exception as e:
                    log.err(f"[ERROR][cp.py] Failed to copy realfile {original_realfile} to {new_realfile}: {e}", system="cowrie")
                    # optional: you might want to fallback and not change realfile if copying failed

                # Update the entry to point to the new real file
                s[fs.A_REALFILE] = new_realfile
                log.msg(f"[DEBUG][cp.py] Updated s[fs.A_REALFILE] to {new_realfile}", system="cowrie")

                # Redirection for terminal tracking
                self.protocol.terminal.redirFiles.add((new_realfile, outfile_path))
                log.msg(f"[DEBUG][cp.py] Added redirected file (new copy): {(new_realfile, outfile_path)}", system="cowrie")

                # Authorized keys handling
                if self.fs.extract_username_from_authorized_keys(outfile_path):
                    self.fs.update_size(outfile_path, s[fs.A_SIZE])
...
class Command_mv(HoneyPotCommand):
    ...
    def call(self) -> None:
        ...
        for src in sources:
            ...
            log.msg(f"[DEBUG][commands.py][fs.py] Moving file: {outfile_path}", system="cowrie")
            log.msg(f"[DEBUG][commands.py][fs.py] FROM file: {resolv(src)}", system="cowrie")
            ...
            if s[fs.A_REALFILE]:
                old_entry = (s[fs.A_REALFILE], resolv(src))
                if old_entry in self.protocol.terminal.redirFiles:
                    self.protocol.terminal.redirFiles.remove(old_entry)
                    log.msg(f"[DEBUG][cp.py] Removed old redirected file entry: {old_entry}", system="cowrie")

                self.protocol.terminal.redirFiles.add((s[fs.A_REALFILE], outfile_path))
                log.msg(f"[DEBUG][cp.py] Added redirected file: {(s[fs.A_REALFILE], outfile_path)}", system="cowrie")
