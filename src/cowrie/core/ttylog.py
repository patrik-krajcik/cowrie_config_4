...
def ttylog_open(logfile: str, stamp: float) -> None:
    ...
    log.msg(f"[DEBUG][ttylog.py][ttylog_open] Opened tty log: {logfile} at {stamp}", system="cowrie")
...
def ttylog_write(
    ...
    log.msg(f"[DEBUG][ttylog.py][ttylog_write] Wrote {length} bytes (dir={direction}) to {logfile} at {stamp}", system="cowrie")
...
def ttylog_close(logfile: str, stamp: float) -> None:
    ...
    log.msg(f"[DEBUG][ttylog.py][ttylog_close] Closed tty log: {logfile} at {stamp}", system="cowrie")
...
def ttylog_inputhash(logfile: str) -> str:
    ...
    @param logfile: logfile name
    """
    log.msg(f"[DEBUG][ttylog.py][ttylog_inputhash] Computing SHA256 hash for {logfile}", system="cowrie")
    ...
    with open(logfile, "rb") as fd:
        ...
        log.msg(f"[DEBUG][ttylog.py][ttylog_inputhash] Hash for {logfile}: {shasum}", system="cowrie")
        return shasum
